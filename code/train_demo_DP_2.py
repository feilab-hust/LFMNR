...
def get_block_dp(pts, model, embedder, post=torch.relu, chunk=32*1024, model_kwargs={'embedder':{}, 'post':{}}):
    sh = pts.shape[:-1]
    ...
    for i in range(0, pts.shape[0], chunk):
        ...
        h = embedder.embed(pts_chunk, **embedder_kwargs)
        h = model(h)
        h = post(h)
        outs.append(h)
...
def train_Static(Flags, meta: dict = None, models_created=None, dataset_preloaded=None,
    ...
    # print('Current_inten of view1:','mean-%.6f  min-%.6f  max-%.6f'%(targets[0].mean(),targets[0].min(),targets[0].max()))
    ## Grid building
    ...
    model, embedder, post_processor, optimizer, start,embedder_args, post_args = models_created
    ...
    for step in rg(start + 1, N_steps_pre + 1):
        ...
        block_re = get_block_dp(pts_glb_re, model, embedder, post_processor, Flags.chunk)
        ...
        if (step % Flags.i_weights == 0 and seq_first) or (step % 50 == 0 and not seq_first):
            ...
            model_dict = {
                'global_step': step,
                'network_fn_state_dict': model.module.get_state(),
        ...
        if (step % Flags.i_print == 0 and seq_first) or (step % 50 == 0 and not seq_first):
            with torch.no_grad():
                ...
                voxel = get_block_dp(pts_glb_all, model, embedder, post_processor, chunk=Flags.chunk)
    ...
    for step in rg(st_second + 1, Nsteps + 1):
        ...
        block = get_block_dp(pts_glb, model, embedder, post_processor, Flags.chunk)
...
def train_seq(Flags):
    ...
    # image normalization for retaining grayscale distribution
    ...
    for t_idx in range(1, T):
        ...
        if isinstance(models_load,tuple):
            # if found ckpt: rePack the model
            models_load = list(models_load)
            models_load[0] = torch.nn.DataParallel(models_load[0].get_model(), device_ids=device_list)
        # replace model for sequential transfer
