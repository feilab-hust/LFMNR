...
def train_Static(Flags, meta: dict = None, models_created=None, dataset_preloaded=None,
    ...
    for step in rg(start + 1, N_steps_pre + 1):
        ...
        targs_re = ds_targets[:,
                   ...
                   :]
        preds_re = torch.stack([forward_project(block_re, _h, padding=padding) for _h in ds_H_mtxs], dim=0)
        ...
        if Flags.add_ssim_loss:
            if 'ssim' in dataset:
                preds_ssim_re = torch.stack(
                    [forward_project(block_re, _h_ssim, padding=padding) for _h_ssim in ds_H_mtxs_ssim], dim=0)
        ...
        if (step % Flags.i_print == 0 and seq_first) or (step % 50 == 0 and not seq_first):
            with torch.no_grad():
                ...
                image_rs = torch.stack([forward_project(voxel, H_mtx) for H_mtx in H_mtxs], dim=0)
                targs = targets
                ...
                if 'H_mtxs_ssim' in locals():
                    targs_ssim = dataset['ssim']['targets']
                    image_rs_ssim = torch.stack([forward_project(voxel, H_mtx_ssim) for H_mtx_ssim in H_mtxs_ssim])
                ...
                if Flags.shot_when_print:
                    ...
                    tifffile.imwrite(os.path.join(Flags.basedir, Flags.expname, 'recon', f'vox_{(step - 1):06d}.tif'),
                                     Normalize_data(torch.relu(voxel).cpu().numpy()[::-1], cast_bitdepth=16))
    ...
    ##          CORE LOOP          ##
    #################################
    for step in rg(N_steps_pre + 1, Nsteps + 1):
            ...
            if step % Flags.ErMap_update == 0 or step == N_steps_fine + 1:
                with torch.no_grad():
                    ...
                    if 'ssim' in dataset:
                        ...
                        temp_pred = torch.stack([forward_project(voxel, H_mtx_ssim) for H_mtx_ssim in H_mtxs_ssim])
                        center_preds = temp_pred[3, ...]
                    else:
                        center_view = targets[3, ...]
                        temp_pred = torch.stack([forward_project(voxel, H_mtx) for H_mtx in H_mtxs], dim=0)
                        center_preds = temp_pred[3, ...]
        ...
        preds = torch.stack([forward_project(block, H_mtx, padding=padding) for H_mtx in H_mtxs], dim=0)
        ...
        if Flags.add_ssim_loss:
            if 'ssim' in dataset:
                ...
                preds_ssim = torch.stack(
                    [forward_project(block, H_mtx_ssim, padding=padding) for H_mtx_ssim in H_mtxs_ssim], dim=0)
            else:
...
def train_dynamic(Flags, meta: dict = None, models_created=None, dataset_preloaded=None,
    ...
    l, r, n, f, padl, padr, padn, padf = 0, 0, 0, 0, 0, 0, 0, 0
    # arggetter = {
    #     'Hts': {'getter': lambda: dataset.get('Hts'), 'dynamic': False},
    #     'views': {'getter': lambda: dataset.get('views'), 'dynamic': False},
    #     'bdbox': {'getter': lambda: (l + padl, r - padr, n + padn, f - padf, 0, D - 1), 'dynamic': True},
    #     't': {'getter': lambda: meta['t'], 'dynamic': False}
    # }
    # # gather additional inputs other than pts for model
    # model_kwargs = {
    #     'embedder': {argname: arggetter[arg]['getter']() for argname, arg in embedder_args.items()},
    #     'post': {argname: torch.tensor(arggetter[arg]['getter'](), device=DEVICE) for argname, arg in post_args.items()}
    # }
    # for key, val in model_kwargs['embedder'].items():
    #     if val is Iterable:
    #         model_kwargs[key] = torch.tensor(val, device=DEVICE)
    # for key, val in model_kwargs['post'].items():
    #     if val is Iterable:
    #         model_kwargs[key] = torch.tensor(val, device=DEVICE)
    ...
    ## run
    if Flags.action.lower() == 'train': print("Start training...")
    if writer is None: writer = SummaryWriter(log_dir='./logs/%s/Tensorboard' % Flags.expname)
